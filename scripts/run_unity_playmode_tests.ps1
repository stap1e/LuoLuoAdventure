<#
.SYNOPSIS
    Run Unity PlayMode tests in batchmode and parse results.
.DESCRIPTION
    Runs Unity 2022.3.62f3 batchmode -runTests for PlayMode.
    IMPORTANT: Does NOT pass -quit. Unity -quit causes exit before Test Runner starts.
    Instead, Unity's Test Runner will exit the process itself when tests complete.
...
.PARAMETER TimeoutSeconds
    Max seconds to wait for Unity. Default 900 (15 min). 0 = infinite.
.EXAMPLE
    .\run_unity_playmode_tests.ps1
#>
[CmdletBinding()]
param(
...
$XmlPath = Join-Path $ResultsDir "playmode-results.xml"
$LogPath = Join-Path $ResultsDir "playmode-editor.log"
...
Write-Host "=== Unity PlayMode Test Run ===" -ForegroundColor Cyan
